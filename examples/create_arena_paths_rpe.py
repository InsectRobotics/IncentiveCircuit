...
def main(*args):
    ...
    if read_arg(["-b", "--only-b"], args=args):
        ...
        only_b = [True, False]
    rng = np.random.RandomState(read_arg(["--rng", "--random-state"], vtype=int, default=202, args=args))
    ...
    for punishment, susceptible, reciprocal, ltm, only_a, only_b in zip(
        ...
        for repeat in range(repeats):
            ...
            for i in range(nb_flies):
                if len(flies) <= i:
                    fly = FruitFly(rng=rng, nb_steps=nb_timesteps, learning_rule="rpe", gain=0.05, nb_kcs=nb_kcs,
