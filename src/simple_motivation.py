...
class MotivationModel(MBModel):
    def __init__(self, *args, **kwargs):
        kwargs.setdefault("nb_dan", 4)
        kwargs.setdefault("nb_mbon", 4)
        ...
        #     DANs                          MBONs
        # --------------------------------------------------
        # 00: sadness (γ1ped)           04: joy (γ1ped)
        # 01: joy (PAM-γ4<γ1γ2)         05: sadness (γ4>γ1γ2)
        # 02: disapproval (PPL1-γ2α'1)  06: optimism (γ2α'1)
        # 03: optimism (PAM-β'2a)       07: disapproval (γ5β'2a)
        self._w_m2v = np.zeros((nb_neurons, nb_neurons), dtype=float)
        self._w_m2v[p_mbon_pri:p_mbon_sec, p_dan_prim:p_dan_seco] = -np.eye(2)  # primary emotions depress their DANs
        ...
        self.names[0], self.names[1], self.names[2], self.names[3] = (
            "PPL1-γ1ped", "PAM-γ4<γ1γ2", "PPL1-γ2α'1", "PAM-β'2a")
        self.names[4], self.names[5], self.names[6], self.names[7] = (
...
def plot_population(ms, nids=None):
    title = "simple-motivation-" + '-'.join(str(ms[0]).split("'")[1:-1:2])
...
if __name__ == '__main__':
    ...
    val, acc, prediction, models = evaluate(model, nids=neurons, behav_mean=target,  behav_std=target_s,
                                            cs_only=True, reversal=True, no_shock=True)
