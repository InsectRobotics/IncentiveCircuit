...
class TMaze(object):
    def __init__(self, train=None, test=None, nb_train=5, nb_test=2, nb_in_trial=100, nb_samples=100,
                 nb_pn=4, nb_kcs=20, nb_kc_odour=6, learning_rule="dpr", rng=np.random.RandomState()):
        """
        ...
        for s in range(nb_samples):
            self.mb.append(IncentiveCircuit(
                nb_pn=nb_pn, nb_kc=nb_kcs, nb_kc_odour=nb_kc_odour, nb_active_kcs=nb_kcs // 2,
        ...
        for mb in self.mb:
            ...
            if train:
                ...
                for i in ids:
                    ...
                    v_out[:nb_v_train] = np.mean(v_i[:, 6::2] - v_i[:, 5::2], axis=1) / 2
